...
#
# Software adicional:
#   - Se selecciona interactivamente con tasksel
#   - Recomendado:
#     [*] SSH server (openssh-server)
#     [*] standard system utilities (vim, curl, man, etc.)
#
# Usuarios:
...
#      - Guardar como: E:\scripts\debian-btrfs-installer.sh
...
#
#   8. Ejecutar:
#      chmod +x debian-btrfs-installer.sh
#      sudo ./debian-btrfs-installer.sh
#
...
#    mv installer.sh debian-btrfs-installer.sh
#
# 3. Ejecutar:
#    chmod +x debian-btrfs-installer.sh
#    sudo ./debian-btrfs-installer.sh
...
SUGGESTED_SWAP=""
SWAP_REASON=""
SUGGESTED_TIMEZONE="America/Mexico_City"
SUGGESTED_LOCALE="es_MX.UTF-8"
...
check_requirements() {
    ...
    for tool in debootstrap gdisk mkfs.btrfs mkfs.fat; do
    ...
    if [[ "$tools_needed" == true ]]; then
        ...
        apt update &>/dev/null
        apt install -y debootstrap gdisk btrfs-progs dosfstools &>/dev/null
    fi
...
interactive_config() {
    ...
    while [[ "$config_valid" == false ]]; do
        ...
        SYSTEM_SIZE_NUM=$(echo "$SYSTEM_SIZE" | sed 's/[^0-9]//g')
...
deb-src $DEBIAN_MIRROR ${DEBIAN_RELEASE}-updates main contrib non-free non-free-firmware
EOF
...
install_software() {
    separator
    echo "SELECCIÓN DE SOFTWARE"
    separator
    echo ""
    echo "A continuación se abrirá tasksel."
    echo ""
    echo "Recomendaciones:"
    echo "  [*] SSH server"
    echo "  [*] standard system utilities"
    echo ""
    echo "Navegación: FLECHAS, ESPACIO, TAB, ENTER"
    echo ""
    read -p "Presiona ENTER..." dummy
...
create_swapfile() {
    ...
    local swap_mb=$(echo "$SWAP_SIZE" | sed 's/G/000/')
